...
from infrastructure.minio_client import download_pdf_bytes, is_minio_available, list_pdfs
...
LOCAL_PDF_PATH = Path(__file__).resolve().parents[1] / "rawinfosmart.pdf"
...
def load_all_pdf_texts() -> list[tuple[str, str]]:
    ...
    if is_minio_available():
        ...
        pdf_names = list_pdfs()
        if not pdf_names:
            print(f"[WARN] No PDFs in bucket '{settings.minio_bucket}'. Falling back to local.")
        else:
