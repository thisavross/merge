...
class Settings(BaseSettings):
    ...
    chatbot_secret: str = Field(default="", description="Must match Moodle X-Chatbot-Secret if set")
    ...
    # ── RAG / chunking ────────────────────────────────────────────────────────
    chunk_size: int = Field(default=800, description="Chat collection chunk size (chars).")
    chunk_overlap: int = Field(default=100, description="Overlap between chat chunks (chars).")
    max_chunks: int = Field(default=40)
    top_k: int = Field(default=5, description="Chat retrieval: chunks per course query.")

    quiz_chunk_size: int = Field(default=1200, description="Quiz collection chunk size (chars).")
    quiz_chunk_overlap: int = Field(default=150, description="Overlap between quiz chunks (chars).")
    quiz_max_chunks: int = Field(default=60, description="Max quiz chunks stored per course.")
    quiz_top_k: int = Field(default=20, description="Quiz retrieval: initial Chroma hits.")
